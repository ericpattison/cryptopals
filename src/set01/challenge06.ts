import { decodeBase64, encodeAscii } from '../enc';
import { xor, score } from '../crypt';
import { hammingDistance } from '../hamming'

const testData = decodeBase64('');

let subsets = (l, a) => {
    return [
        a.slice(0,l),
        a.slice(l,l+l)
    ];
}

let keySizes = [];
for(let keySize = 2; keySize < 40; ++keySize) {
    let s = subsets(keySize, testData);
    let d = hammingDistance(s[0], s[1]);
    keySizes.push({
        distance: d,
        size: keySize,
        normal: d/keySize
    });
}
keySizes.sort((a,b) => {return a.normal - b.normal});

/*
let blocks = [];
for(let i = 0; i < keySizes.length; ++i) {
    let size = keySizes[i].size;
    let testSet = [];
    for(let j = 0; j < testData.length; ++j) {
        testSet[j%size] = testSet[j%size] || [];
        testSet[j%size].push(testData[j]);
    }
    blocks.push({
        keySize: size,
        data: testSet
    });
}

let key = [];
blocks.forEach((block) => {
    let scores = [];
    for(let i = 0; i < 256; ++i) {
        block.data.forEach((dataSet) => {
            let msg = encodeAscii(xor(i, dataSet));
            scores.push({
                key: i,
                message:msg,
                score: score(msg)
            })
        });
    }

    scores.sort((a,b) => { return b.score - a.score; });
    key.push(scores[0].key);
})

console.log(encodeAscii(key));
*/